...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop):
        ...
        self.rat_mode = False
        self.rat_mode_side = -1 # Side.BUY if it needs to buy, Side.SELL if it needs to sell
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        def order_quantity(side):
            """trader_stance is a boolean: True = passive, False = aggressive. 
            side is order side (buy or sell). True = buy, False = sell"""
            volume = int((abs(sum(bid_volumes)-sum(ask_volumes))/10000) * 3 * (self.number_of_matches_in_tick))
        ...
        if len(bid_prices) > 0 and len(ask_prices) > 0:
            if instrument == Instrument.FUTURE:
                ...
                if self.predicted_hedge_price % 100 == 0:
                    bid_price = int(self.predicted_hedge_price - 100)
                    ask_price = int(self.predicted_hedge_price + 100)
                else:
                    bid_price = int(self.predicted_hedge_price - 50)
                    ask_price = int(self.predicted_hedge_price + 50)
                
                for key in list(self.active_order_history.keys()):
                    if self.active_order_history[key][4] == Side.BUY and self.active_order_history[key][2] != bid_price:
                        self.op_send_cancel_order(key)
                    elif self.active_order_history[key][4] == Side.SELL and self.active_order_history[key][2] != ask_price:
                        self.op_send_cancel_order(key)
                
                if self.rat_mode:
                    current_active_order_position = check_active_order_position() # The position in either direction if all orders of that side go through

                    if (self.rat_mode_side == Side.BUY and current_active_order_position[0] < 0) or (self.rat_mode_side == Side.SELL and current_active_order_position[1] > 0): # If the current orders aren't moving the position towards 0
                        self.order_id = next(self.order_ids)
                        if self.rat_mode_side == Side.BUY:
                            self.op_send_insert_order(self.order_id, Side.BUY, bid_price, abs(current_active_order_position[0]), Lifespan.GOOD_FOR_DAY)
                        else:
                            self.op_send_insert_order(self.order_id, Side.SELL, ask_price, abs(current_active_order_position[1]), Lifespan.GOOD_FOR_DAY)
                            
                        if (self.rat_mode_side == Side.BUY and self.position > -10) or (self.rat_mode_side == Side.SELL and self.position < 10):
                            self.rat_mode = False
                    
                elif self.get_projected_op_rate(2) <= 19.5:
                    self.bid_id = next(self.order_ids)
                    self.op_send_insert_order(self.bid_id, Side.BUY, bid_price, 10, Lifespan.GOOD_FOR_DAY)
                    self.ask_id = next(self.order_ids)
                    self.op_send_insert_order(self.ask_id, Side.SELL, ask_price, 10, Lifespan.GOOD_FOR_DAY)


        if (self.rat_mode_side == Side.BUY and self.position > -10) or (self.rat_mode_side == Side.SELL and self.position < 10):
            self.rat_mode = False
            self.rat_mode_side = -1
    ...
    def on_position_change_message(self, future_position: int, etf_position: int) -> None:
        ...
        if abs(self.position) > 90:
            self.rat_mode = True
            
            if self.position < 0:
                self.rat_mode_side = Side.BUY
            else:
                self.rat_mode_side = Side.SELL

            for key in list(self.active_order_history.keys()):
                if self.active_order_history[key][0] != self.rat_mode_side:
                    self.op_send_cancel_order(key)
