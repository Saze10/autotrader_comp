        ...
        if (ask_volumes[0] != 0 and bid_volumes[0] != 0 and len(self.trade_tick_list) > 0):
            ...
            if volume_difference > 0.5 and self.get_projected_op_rate(2) <= 19.5: # Aggressive strategy                
                ...
                if order_volume > 0:
                    ...
                    # function returns True when it replaces an order
                    if not check_same_price_order(Side.SELL, ask_trading_price, self.ask_id, order_volume):
                ...
                if order_volume > 0:
                    ...
                    if not check_same_price_order(Side.BUY, ask_trading_price, self.ask_id, order_volume):
                ...
                if self.get_projected_op_rate(2) <= 19.5:
                    ...
                    if order_volume > 0:
                        self.bid_id = next(self.order_ids)
                        if not check_same_price_order(Side.BUY, bid_trading_price, self.bid_id, order_volume):
                            self.op_send_insert_order(self.bid_id, Side.BUY, bid_trading_price, order_volume , Lifespan.GOOD_FOR_DAY)
